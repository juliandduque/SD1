...
function createRecord($dbConnection, $jsonPayload)
{
  ...
  // This block uses prepared statements and parameterized queries to protect against SQL injection
		...
		file_put_contents("php://stderr", "INSERT INTO `chatterBox`.`data` (`datetime`, `Frequency`, `deviceID`, `key`) VALUES ('".$time."', '".$frequency."', '".$deviceID."', '1234')");
        $query = $dbConnection->prepare("INSERT INTO `chatterBox`.`data` (`datetime`, `Frequency`, `deviceID`, `key`) VALUES ('".$time."', '".$frequency."', '".$deviceID."', '1234')");
        $query->execute();
